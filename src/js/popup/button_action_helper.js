...
import { startProjectTimer, stopActivityTimer, restartActivityTimer } from './api_call_helper';
...
export const activityRestartBtnOnClick = e => {
  const activityID = $(e.target).attr('name');
  restartActivityTimer(activityID,
    () => {
      loadAllTimeEntries(renderTimeEntries, showError);
    },
    showError);
